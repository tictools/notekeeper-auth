...
import { NOTE_STATUS, Note } from "../types";
...
describe("NotesInMemoryRepository", () => {
  const mockNote1 = {
    _id: "mockNote1-6d1d-49fb-ae67-0e8ab50d6554" as UUID,
    name: "Walk the dog",
  ...
  const mockNote2 = {
    _id: "mockNote2-46ca-42b0-a48a-b971e6603fe0" as UUID,
    name: "Buy groceries",
    description: "Get milk, eggs, and bread",
    important: true,
    status: NOTE_STATUS.IN_PROGRESS,
    due_date: "6/1/2024",
    created_at: 1714552849903,
  };

  const mockUpdatedNote = {
    ...mockNote1,
    _id: "mockUpdatedNote-6d1d-49fb-ae67-0e8ab50d6554" as UUID,
  };
  ...
  const repository = InMemoryNotesRepository(mockNotes);
  ...
  describe("Given findIndexNoteById", () => {
    test("When method is called and index does not exist Then index -1 is returned", async () => {
      const noteIndex = await repository.findIndexNoteById(mockNote1._id);
    ...
    test("When method is called and index exists Then note index is returned", async () => {
      ...
      const foundNote = (await repository.getNote(noteIndex)) as Note;
